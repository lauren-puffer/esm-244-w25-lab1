...
the file name "trees" and we need to add the csv file. 
...
197618 observations
Includes tree id, legal status, species, address, site order, site info, caretaker, plant date, dbh (?), plot size, latitude, and longitude. 

How does the type of caretaker dictate the species of trees? I.e. do private caretakers prefer certain species of trees?

How many counts of each observation by legal_status? Focus on highest trees?
unique(sfo_trees_df$legal_status) to see distinct categories for legal status
...
```
Make a plot of top 5 categories of tree legal status. 

```{r}
ggplot(data = graph_data, aes(x=fct_reorder(legal_status,tree_count), y=tree_count))+
...
This code chunk filters out only the rows of trees from "permitted site" and those that have caretakers with MTA and DPW. 
...
%in% takes date from a certain column and allows you to select rows that contain certain values. Only returns values for where anything after %in% is TRUE. 

```{r}
sfo_trees_df |>
  filter(legal_status %in% c("DPW Mainatined", "Significant Tree"))
```
...
filter out plants that aren't pine and oak
summarize counts of pines and oaks by legal status


Data wrangle done below:
 | is the "or" argument
 str_detect to detect characters in cells and keep them if TRUE. 
...
lubridate allows you to define years as a number. 
